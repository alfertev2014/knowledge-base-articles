...
В Java разрешено множественное наследование интерфейсов, которое не имеет никаких проблем, потому что интерфейсы специально ограничены в своей функциональности. Даже в случае ромба есть чёткие правила, определяющие, какой метод будет вызван. С приведением типа объекта к типу интерфейса тоже никаких проблем нет - всё однозначно, даже если интерфейс несколько раз встречается в графе наследования интерфейсов.

#### Проблемы запрета множественного наследования

Все объекты обязаны наследоваться от корневого класса иерархии наследования - класса Object. По идее, класс Object не должен содержать никакой специфичной функциональности, потому что ему должны принадлежать все вообще объекты. А на самом деле, получается, что в Java все объекты получают неотключаемую функциональность: equals()/hashCode(), wait()/notify(), возможность быть мьютексом в synchronize, сериализация/десериализация - хотим мы того или нет.
...
В Python 3 множетсвенное наследование разрешено. Существуют правила, по которым выполняется поиск члена класса среди базовых классов.

Чтобы избежать проблемы ромба, в Python для каждого класса, наследующегося от других классов, определяется Method Resolution Order (MRO) - формируется линейный список из всех базовых классов вместе с транзитивно наследуемыми. Порядок наследования так же важет, как в Scala.

Класс, который предназначен для множественного наследования, называется mixin. С точки зрения Python это обычный класс, но для разработчика он не имеет смысла, если не используется в наследовании другим классом. Также, как и в Scala, один mixin может зависеть от других. Если быть точнее, то mixin может предъявлять определённые требования к self-объекту, но это будет путём "утиной" типизации. Часто эти требования так слабо выражены, что легко можно вызвать ошибку при неправильном использовании mixin.

### JavaScript

В JavaScript используется прототипное наследование. У каждого объекта может быть только один прототип, поэтому множественное наследование в чистом виде не представлено. Также, в отличие от Python, нет такой гибкой системы MRO. Есть лишь одно правило: если метод не найден в объекте, то он ищется в его прототипе. На самом деле, выстраивая цепочку прототипов, можно самостоятельно определить порядок, аналогичный MRO в Python.

Что есть mixin? Mixin можно рассматривать как функцию над классами. Mixin принимает на вход класс и конструирует на его основе другой класс. В JavaScript возможно описать наследование от результата применения функции к классу. Всё это будет вести себя почти как множественное наследование за исключением нескольких вещей:

* невозможно определить принадлежность объекта к mixin с помощью instanceof,
* для каждого применения mixin к различным классам буквально конструируются новые фрагменты объектов, включаемые в цепочки наследования. Это в чём-то немного бесполезное дублирование в памяти.
* есть риск повторно применить тот же mixin к классу. Требуется что-то похожее на виртуальное наследование в C++.

Этот подход рассматривается в статье https://dev.to/open-wc/using-javascript-mixins-the-good-parts-4l60

## Наследование и композиция

Когда речь идёт о композиции в предметной области, ожидается, что один объект соотносится с другим именно как часть. В ООП же часто приходится использовать композицию не для моделирования отношения часть-целое, а как технический приём. Есть много паттернов, которые применяют композицию, хотя семантически ничего общего не имеют с отношением часть-целое в предметной области.

### Кусок класса - mixin

### Функции и extention methods
